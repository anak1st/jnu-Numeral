...
def spline3(x, y, l, r, op):
    ...
    if op == 4:
        for i in range(n - 1):
            x3 = (M[i + 1] - M[i]) / (6 * h[i + 1])
            
            x2 = (M[i] * x[i + 1] - M[i + 1] * x[i]) / (2 * h[i + 1])
            
            x1 = 0
            x1 += h[i + 1] * (M[i] - M[i + 1]) / 6
            x1 += (M[i + 1] * x[i] ** 2 - M[i] * x[i + 1] ** 2) / (2 * h[i + 1])
            x1 += (y[i + 1] - y[i]) / h[i + 1]
    
            x0 = 0 
            x0 += h[i + 1] * M[i + 1] * x[i] / 6
            x0 -= M[i + 1] * x[i] ** 3 / (6 * h[i + 1])
            x0 -= h[i + 1] * M[i] * x[i + 1] / 6
            x0 += M[i] * x[i + 1] ** 3 / (6 * h[i + 1])
            x0 += x[i + 1] * y[i] / h[i + 1]
            x0 -= x[i] * y[i + 1] / h[i + 1]

            print(f"{x0:.5f} {x1:.5f} {x2:.5f} {x3:.5f}")
